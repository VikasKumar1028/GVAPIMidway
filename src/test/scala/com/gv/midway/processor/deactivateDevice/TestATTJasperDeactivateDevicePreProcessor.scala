...
class TestATTJasperDeactivateDevicePreProcessor extends TestMocks with ATTJasperSuite {
  ...
  test("process") {
    withMockExchangeMessageAndEnvironment { (exchange, message, environment) =>
      ...
      val id1 = new DeactivateDeviceId
      id1.setId("id1")
      ...
      val payload = new DeactivateDeviceRequest
      ...
      new ATTJasperDeactivateDevicePreProcessor(environment).process(exchange)
