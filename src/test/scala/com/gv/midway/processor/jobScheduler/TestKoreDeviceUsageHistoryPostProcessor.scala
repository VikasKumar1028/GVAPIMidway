...
class TestKoreDeviceUsageHistoryPostProcessor extends FunSuite with MockitoSugar {
  ...
  private val validMap = getValidMap
  ...
  test("KoreDeviceUsageHistoryPostProcessor - single day match") {
    ...
    withMockExchangeAndMessage(validMap) { (exchange, message) =>
  ...
  private def withMockExchangeAndMessage(map: JMap[_, _])(f: (Exchange, Message) => Unit): Unit = {
    ...
    when(message.getBody(any)).thenReturn(map, Nil: _*)

    f(exchange, message)
  }

  private def getValidMap: JMap[_, _] = {
    import collection.JavaConversions._
    val usage1 = new Usage()
    usage1.set__type("Element 1 type")
    usage1.setDataInBytes(14.0)
    usage1.setSms(10l)
    usage1.setUsageDate("/Date(1475294400000-0400)/")

    val usage2 = new Usage()
    usage2.set__type("Element 2 type")
    usage2.setDataInBytes(15.0)
    usage2.setSms(11l)
    usage2.setUsageDate("/Date(1475380800000-0400)/")  //10-01-2016 - Same as jobDetailDate

    val usage3 = new Usage()
    usage3.set__type("Element 3 type")
    usage3.setDataInBytes(100.0)
    usage3.setSms(0l)
    usage3.setUsageDate("/Date(1478467200000-0400)/")

    val list = List(usage1, usage2)
    val d = new D
    d.setUsage(list)
    ...
    mapper.convertValue(uikr, classOf[JMap[_, _]])
  }
}