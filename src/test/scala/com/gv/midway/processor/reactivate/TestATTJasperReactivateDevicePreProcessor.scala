...
class TestATTJasperReactivateDevicePreProcessor extends TestMocks with ATTJasperSuite {
  ...
  test("process") {
    ...
    withMockExchangeMessageAndEnvironment { (exchange, message, environment) =>
      ...
      val payload = new ReactivateDeviceRequest
      ...
      new ATTJasperReactivateDevicePreProcessor(environment).process(exchange)
