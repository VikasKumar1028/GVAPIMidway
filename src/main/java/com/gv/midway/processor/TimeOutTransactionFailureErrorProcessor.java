...
public class TimeOutTransactionFailureErrorProcessor implements Processor {

	private static final Logger LOGGER = Logger
			.getLogger(TimeOutTransactionFailureErrorProcessor.class.getName());

	Environment newEnv;

	public TimeOutTransactionFailureErrorProcessor(Environment env) {
		super();
		this.newEnv = env;

	}

	public TimeOutTransactionFailureErrorProcessor() {
		// Empty Constructor
	}
	...
	public void process(Exchange exchange) throws Exception {
		...
		if (jobName.endsWith("DeviceUsageJob")) {
			...
			DeviceUsage deviceUsage = (DeviceUsage) exchange.getIn().getBody();
