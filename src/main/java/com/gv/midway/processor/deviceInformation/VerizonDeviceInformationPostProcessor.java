...
public class VerizonDeviceInformationPostProcessor implements Processor {
	...
	private static final Logger LOGGER = Logger
			.getLogger(VerizonDeviceInformationPostProcessor.class.getName());
	...
	public void process(Exchange exchange) throws Exception {
		...
		if (getDevicelenth > 0) {
			...
			/****
			 * There will be only one element in Array . As we are passing only
			 * one device in request.
			 */
			for (int i = 0; i < 1; i++) {

				deviceInformation
						.setAccountName(verizonResponse.getDevices()[i]
								.getAccountName());
				...
				deviceInformation.setLastActivationBy(verizonResponse
						.getDevices()[i].getLastActivationBy());
		 ...
		 */
			...
			DeviceId deviceId = (DeviceId) exchange
					.getProperty(IConstant.MIDWAY_DEVICE_ID);
			...
			if (deviceIdsPrev == null || deviceIdsPrev.length == 0) {
				...
				for (int i = 0; i < deviceIdsPrev.length; i++) {
					...
					if (deviceIdPrev.getKind().equalsIgnoreCase(
							deviceId.getKind())) {
