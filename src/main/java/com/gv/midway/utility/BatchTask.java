...
public class BatchTask {
	...
	public BatchTask(MongoTemplate mongoTemplate, String action/*
																 * ,Devices
																 * devices
																 */) {
	...
	public Object doBatchJob() {
		...
		CompletionService<Object> compService = new ExecutorCompletionService<Object>(
				executorService);
		/*
		 ...
		 * 
		 * public Object call() throws Exception { // TODO Auto-generated method
		 * stub
		 * 
		 * try { device.setStatus("activate"); //
		 * device.setIpAddress("127.0.0.1"); SimpleDateFormat sdf = new
		 * SimpleDateFormat( "yyyy-MM-dd'T'HH:mm:ssZ");
		 * device.setLastUpdated(sdf.format(new Date()));
		 * 
		 * mongoTemplate.insert(device); }
		 * 
		 * catch (Exception e) { failCount=failCount+1; Cell
		 * cell=device.getCell(); error_cells.add(cell); return cell; }
		 * 
		 * successCount=successCount+1; InsertCell insertCell= new InsertCell();
		 * insertCell.setId(device.getId()); insert_cells.add(insertCell);
		 * return insertCell; } }); }
		 * 
		 * 
		 * for (final Device device : deviceArr) {
		 * 
		 * try { compService.take().get(); // find the first completed task
		 * 
